...
Figure \ref{fig:3.3} shows an Long Short Term Memory (LSTM) unit.
\begin{figure}[H]
\centering
...
\begin{align}
\Gamma _u = \sigma (W_u \cdot [x_t, h_{t-1}]) \\
\Gamma _f = \sigma (W_f \cdot [x_t, h_{t-1}]) \\
\Gamma _o = \sigma (W_o \cdot [x_t, h{t-1}])
\end{align}
